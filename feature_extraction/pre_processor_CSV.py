...
class csvProcessor():
    def __init__(self, CSV_paths: list):
        self.CSVs = [] # Store the individual CSV files.
        for path in CSV_paths:
            self.CSVs.append(pd.read_csv(path))
    
        self.combined_CSV = None # Store the combined CSV.
    ...
    '''Method that, given a list of reference classes, drops the non-reference classes from the combined CSV.'''
    ...
    '''Method that writes the combined CSV to a file.'''
